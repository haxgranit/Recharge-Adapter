...
(function ($, ns) {
    ...
    var get = function (origin, target) {
        ...
        $(target).each(function () {
            ...
            for (
                var current = prev.parent();
                current.length && !prev.is(origin);
...
(function ($, ns) {
    ...
    var EXTENDED_OPTIONS = toObject(["added", "removed"], function (name) {
        return [name.toLowerCase(), name];
    });
    ...
    Pattern.prototype.is = function (options, selector, handler) {
        return (
            objectToString(this._originalOptions) === objectToString(options) &&
            this.selector === selector &&
            this.handler === handler
        );
    };
    ...
    Pattern.prototype.match = function (record) {
        ...
        if (this.selector) {
            ...
            var recordTarget =
                record.target instanceof Text ? $(record.target).parent() : $(record.target);
            ...
            switch (type) {
                ...
                case "childList":
                    if (
                        (record.addedNodes && record.addedNodes.length && options.added) ||
    ...
    Pattern.prototype._matchSelector = function (origin, element) {
        ...
        match = match.filter(function () {
            ...
            return element.some(function (node) {
                ...
                return node === self || $(node).has(self).length;
            });
        });
    ...
    Observer.prototype._collapseOptions = function () {
        ...
        this.patterns.forEach(function (pattern) {
            ...
            if ((restrictiveFilter || !result.attributes) && pattern.attributeFilter) {
                var attributeFilter = (result.attributeFilter || []).concat(
                    pattern.attributeFilter
                );
                var existing = {};
                var unique = [];
